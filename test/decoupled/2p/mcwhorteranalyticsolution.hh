...
/*****************************************************************************
 *   Copyright (C) 2013 by Markus Wolff                                      *
 *
...
//             std::cout<<" k = "<<k<<std::endl;
            ...
            {
                a_[i] = 0.5 * h_ * sInit_ *(gk_[i] + gk_[i+1])+ std::pow(h_, 2) / 6* ((3* i + 1) * gk_[i]
                        + (3 * i + 2) * gk_[i+1]);
            ...
            Ak = std::pow((0.5*porosity_/std::pow((1 - fInit_), 2)*I0), 0.5);
            diff=std::abs(Ak - Akm1);
        ...
        for (int i = 0; i<pointNum_; i++)
        {
            xf_[i]= 2 * Ak * (1 - fInit_ * r_)/ porosity_ * fp_[i]* std::pow(time, 0.5);
        }
