 ...
 *****************************************************************************/
#ifndef EWOMS_FVSATURATION2P_HH
#define EWOMS_FVSATURATION2P_HH

#include <ewoms/decoupled/2p/transport/transportproperties2p.hh>
#include "ewoms/decoupled/common/fv/fvtransport.hh"
 ...
 *
 *  \f[
 *  \phi \frac{\partial (\rho_\alpha S_\alpha)}{\partial t} + \mathrm{div}\, (\rho_\alpha \boldsymbol{v_\alpha}) = q_\alpha,
 *  \f]
 *
 ...
 *
 * \f[
 *  \phi \frac{\partial S_w}{\partial t} + f_w \mathrm{div}\, \boldsymbol{v}_{t} + f_w \lambda_n \boldsymbol{K}\left(\mathbf{grad}\, p_c + (\rho_n-\rho_w) \, g \, \mathbf{grad} z \right)= q_\alpha,
 * \f]
 * to get a wetting phase saturation or
 * \f[
 * \phi \frac{\partial S_n}{\partial t} + f_n \mathrm{div}\, \boldsymbol{v}_{t} - f_n \lambda_w \boldsymbol{K}\left(\mathbf{grad}\, p_c + (\rho_n-\rho_w) \, g \, \mathbf{grad} z \right)= q_\alpha,
 * \f]
    ...
    if (bcType.isDirichlet(satEqIdx))
    {
        ...
        else
        {
            if (compressibility_)
            {
                ...
                paramCache.updateAll(cellDataI.fluidState());
                lambdaW = MaterialLaw::krw(problem_.spatialParams().materialLawParams(*elementI), satWBound)
                        / FluidSystem::viscosity(cellDataI.fluidState(), paramCache, wPhaseIdx);
            }
            else
            {
                lambdaW = MaterialLaw::krw(problem_.spatialParams().materialLawParams(*elementI), satWBound)
        ...
        else
        {
            if (compressibility_)
            {
                ...
                lambdaNW = MaterialLaw::krn(problem_.spatialParams().materialLawParams(*elementI), satWBound)
                         / FluidSystem::viscosity(cellDataI.fluidState(), paramCache, nPhaseIdx);
            }
            else
            {
                lambdaNW = MaterialLaw::krn(problem_.spatialParams().materialLawParams(*elementI), satWBound)
