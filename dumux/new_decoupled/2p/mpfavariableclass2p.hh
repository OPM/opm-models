...
private:
    ...
    typedef Dune::FieldVector<Scalar, numPhase> PhaseVector;
    typedef Dune::FieldVector<PhaseVector, 2*dim> CornerCellsPhaseVector;
    typedef Dune::FieldVector<CornerCellsPhaseVector, 2*dim> InterfaceCornerCellsPhaseVector;
public:
    typedef Dune::BlockVector<InterfaceCornerCellsPhaseVector> UpwindMobilitiesVector;
...
public:
        ...
        ParentType(gridView, initialSat, initialVel)
    {
        ...
        upwindMobilities_ = CornerCellsPhaseVector(PhaseVector(0));
    }
...
public:
    ...
    Scalar& upwindMobilitiesWetting(int globalIdx, int faceIdx, int cellIdx)
    {
        return upwindMobilities_[globalIdx][faceIdx][cellIdx][wPhaseIdx];
    }

    const Scalar& upwindMobilitiesWetting(int globalIdx, int faceIdx, int cellIdx) const
    {
        return upwindMobilities_[globalIdx][faceIdx][cellIdx][wPhaseIdx];
    }

    //! Return vector of non-wetting phase mobilities
    Scalar& upwindMobilitiesNonwetting(int globalIdx, int faceIdx, int cellIdx)
    {
        return upwindMobilities_[globalIdx][faceIdx][cellIdx][nPhaseIdx];
    }

    const Scalar& upwindMobilitiesNonwetting(int globalIdx, int faceIdx, int cellIdx) const
    {
        return upwindMobilities_[globalIdx][faceIdx][cellIdx][nPhaseIdx];
    }
};
